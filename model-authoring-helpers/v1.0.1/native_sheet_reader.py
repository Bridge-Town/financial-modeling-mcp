...
class NativeSheetNotFoundError(NativeSheetReadError):
    """The requested sheet document does not exist at its canonical path."""
...
class NativeSheetReader:
    ...
    Sheet documents are read only from the public ``sheets/`` location.
    ...
    def _document_path(self, sheet_id: str) -> Path:
        ...
        candidate = self.repo_root / "sheets" / f"{sheet_id}.btsheet.json"
        ...
        raise NativeSheetNotFoundError(
            f"Native Sheet {sheet_id!r} was not found under sheets/.",
            sheet_id=sheet_id,
        )
